...
def resample(
    audio: torch.Tensor, resample_sr: int, original_sr: int
) -> torch.Tensor:
    """
...
class JEPA(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs: dict[str, Any],
        ...
        self.sr = resample_sr
        self.original_sr = original_sr if original_sr is not None else resample_sr
    ...
    def configure_optimizers(self):
        ...
        warmup_steps = int(getattr(self.hparams, "warmup_steps", 100000))
        if self.trainer.max_steps is not None and self.trainer.max_steps > 0:
            warmup_steps = min(warmup_steps, int(self.trainer.max_steps))
    ...
    def prepare_batch(
        ...
        # Signal-only path: treat the input as the scene directly.
        generated_scene = audio
        if generated_scene.ndim == 2:
            # (B, L) -> (B, 1, L)
            generated_scene = generated_scene.unsqueeze(1)
        elif generated_scene.ndim != 3:
            raise ValueError(
                f"Expected audio tensor of shape (B, L) or (B, C, L), got {generated_scene.shape}"
            )

        # 2. Pad/truncate to a fixed window and resample if needed
        generated_scene = self.pad_or_truncate_batch(
            generated_scene, self.target_length
        )
        # Use configured original sample rate.
        if self.sr != self.original_sr:
            generated_scene = self.resample(
                generated_scene, resample_sr=self.sr, original_sr=self.original_sr
            )
