...
MASKERS = {
    ...
    # Alias: configs/masker/MultiBlock.yaml uses "block-masker"
...
class ComponentFactory:
    ...
    @staticmethod
    def create_transformer_cfg(cfg) -> tuple[
        TransformerLayerCFG, TransformerEncoderCFG, TransformerLayerCFG, TransformerEncoderCFG
    ]:
        """Create encoder/decoder transformer configs (optionally overridden via cfg.trainer.*)."""
        enc_d_model = cfg.trainer.get("encoder_d_model", 768)
        enc_nhead = cfg.trainer.get("encoder_nhead", 12)
        enc_num_layers = cfg.trainer.get("encoder_num_layers", 12)
        enc_mlp_ratio = cfg.trainer.get("encoder_mlp_ratio", 4.0)
        enc_dropout = cfg.trainer.get("encoder_dropout", 0.0)

        dec_d_model = cfg.trainer.get("decoder_d_model", 384)
        dec_nhead = cfg.trainer.get("decoder_nhead", 12)
        dec_num_layers = cfg.trainer.get("decoder_num_layers", 12)
        dec_mlp_ratio = cfg.trainer.get("decoder_mlp_ratio", 4.0)
        dec_dropout = cfg.trainer.get("decoder_dropout", 0.0)

        encoder_layer_cfg = TransformerLayerCFG.create(
            d_model=enc_d_model,
            nhead=enc_nhead,
            mlp_ratio=enc_mlp_ratio,
            dropout=enc_dropout,
        )
        encoder_cfg = TransformerEncoderCFG.create(num_layers=enc_num_layers)

        decoder_layer_cfg = TransformerLayerCFG.create(
            d_model=dec_d_model,
            nhead=dec_nhead,
            mlp_ratio=dec_mlp_ratio,
            dropout=dec_dropout,
        )
        decoder_cfg = TransformerEncoderCFG.create(num_layers=dec_num_layers)

        return encoder_layer_cfg, encoder_cfg, decoder_layer_cfg, decoder_cfg
    ...
    def create_masker(cfg):
        ...
        if cfg.masker.name == "inverse-masker":
            ...
            or cfg.masker.name == "multi-block-masker"
            ...
            # MultiBlockMaskMaker supports ratio_cutoff, but Random* maskers do not.
            if cfg.masker.name in ("block-masker", "multi-block-masker"):
                kwargs["ratio_cutoff"] = cfg.masker.get("ratio_cutoff", 0.05)
            return masker_class(
                **kwargs,
            )
    ...
    def create_network(cfg, extractor: Extractor) -> JEPA:
        ...
        try:
            ...
            ) = ComponentFactory.create_transformer_cfg(cfg)

            average_top_k_layers = cfg.trainer.average_top_k_layers
            encoder_num_layers = transformer_encoder_cfg["num_layers"]
            if average_top_k_layers > encoder_num_layers:
                average_top_k_layers = encoder_num_layers

            return network_class(
                feature_extractor=extractor,
                transformer_encoder_cfg=transformer_encoder_cfg,
                transformer_encoder_layers_cfg=transformer_encoder_layers_cfg,
                ...
                warmup_steps=cfg.trainer.get("warmup_steps", 100000),
                in_channels=cfg.data.in_channels,
                resample_sr=cfg.data.sr,
                original_sr=cfg.data.get("original_sr", cfg.data.sr),
